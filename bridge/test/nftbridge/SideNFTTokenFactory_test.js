...
contract("SideNFTTokenFactory", async function(accounts) {
  ...
  const alternateTokenContractURI =
    "https://api-mainnet.rarible.com/alternateContractMetadata";
  ...
  it("should create two correct side NFT tokens upon correct parameters", async function() {
    ...
    receipt = await this.sideNFTTokenFactory.createSideNFTToken(
      alternateTokenName,
      alternateTokenSymbol,
      alternateTokenBaseURI,
      alternateTokenContractURI,
      { from: tokenCreator }
    );
    utils.checkRcpt(receipt);
    truffleAssertions.eventEmitted(
