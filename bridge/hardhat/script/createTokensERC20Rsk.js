...
async function main() {
  ...
  for (const token of tokens) {
    ...
    const methodCallCreateSideToken = bridge.methods.createSideToken(token.typeId, token.originalTokenAddress, tokenDecimals, token.symbol, token.name);
    const receipt = await methodCallCreateSideToken.call({ from: MultiSigWallet.address});
    console.log("Method call receipt", receipt);

    const result = await multiSigContract.methods.submitTransaction(BridgeProxy.address, transactionEtherValue, methodCallCreateSideToken.encodeABI()).send({ from: deployer });
    console.log("Transaction for token:", token, "\n\nworked", result);
  }
  console.log("finish");
}
