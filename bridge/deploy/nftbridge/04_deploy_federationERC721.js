...
module.exports = async function (hre) { // HardhatRuntimeEnvironment
  ...
  if (deployResult.newlyDeployed) {
    log(`Contract FederationV3 with set NFT Bridge deployed at ${deployResult.address} using ${deployResult.receipt.gasUsed.toString()} gas`);
  }

  const federationV3Deployment = await deployments.get('FederationV3');
  ...
  const methodCallUpdagradeFederationDeployment = proxyAdminContract.methods.upgrade(federationProxyDeployment.address, federationV3Deployment.address);
  ...
  const federation = new web3.eth.Contract(federationV3Deployment.abi, federationProxyDeployment.address);
