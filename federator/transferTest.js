...
async function transfer(originFederators, destinationFederators, config, origin, destination) {
    try {
        ...
        await transactionSender.sendTransaction(config.mainchain.federation, '', originWeb3.utils.toWei('1'), config.privateKey);
        await destinationTransactionSender.sendTransaction(config.sidechain.federation, '', originWeb3.utils.toWei('1'), config.privateKey);
