...
const acceptTransferEventAbiDecoder = require('abi-decoder');
...
acceptTransferEventAbiDecoder.addABI([abiAcceptedNFTCrossTransferEvent]);
...
async function getDestinationTokenAddress(destinationBridgeContract, originAddress, sideMultiSigContract,
                                          destinationTransactionSender, config, destinationLoggerName) {
  ...
  if (destinationTokenAddress === utils.zeroAddress) {
    ...
    const data = destinationBridgeContract.methods.createSideToken(
      SIDE_TOKEN_TYPE_ID,
      originAddress,
      SIDE_TOKEN_DECIMALS,
      SIDE_TOKEN_SYMBOL,
    ...
    const multiSigData = sideMultiSigContract.methods.submitTransaction(
      destinationBridgeContract.options.address, 0, data
    ).encodeABI();
...
async function claimTokensFromDestinationBridge(destinationBridgeContract, userAddress, amount, receipt,
                                                destinationTransactionSender, destinationBridgeAddress, userPrivateKey) {
...
async function resetConfirmationsForFutureRuns(allowTokensContract, config, data, methodCall, multiSigContract, allowTokensAddress, cowAddress, mainChainWeb3, confirmations) {
...
async function transferNFT(originFederators, destinationFederators, config, originLoggerName, destinationLoggerName) {
  try {
    ...
    const cowAddress = (await mainChainWeb3.eth.getAccounts())[0]; // TODO: ?
    ...
    let nftBridgeContract = new mainChainWeb3.eth.Contract(abiNFTBridge, originNftBridgeAddress);
    ...
    // logger.debug('Initial user balance ', initialUserBalance); // TODO: since it's like 1000000000000000000, what to do?
    ...
    // TODO: maybe add logs in methods for each phase.
    ...
    {
      // logger.info(`to: ${userAddress}, from: ${userAddress}, tokenId: ${tokenId}, address: ${originTokenAddress}, blockHash: ${receiveTokensToTxReceipt.blockHash}, transactionHash: ${receiveTokensToTxReceipt.transactionHash}, logIndex: ${receiveTokensToTxReceipt.logs[2].logIndex}`);

      const acceptTransferEventDecodedLogs = acceptTransferEventAbiDecoder.decodeLogs(receiveTokensToTxReceipt.logs);
      ...
      const crossEventAddress = acceptTransferEventDecodedLogs[0].address;
      const crossEventLog = receiveTokensToTxReceipt.logs.find(log => log.address === crossEventAddress);
      ...
      const methodCall = destinationNftBridgeContract.methods.claim(
        {
          ...
          blockHash: crossEventLog.blockHash,
          transactionHash: crossEventLog.transactionHash,
          logIndex: crossEventLog.logIndex
        }
      );
      await methodCall.call({from: userAddress});
      const data = methodCall.encodeABI();
      await transactionSender.sendTransaction(destinationNftBridgeContract, data, 0, userPrivateKey, true);
    }

    // TODO: assert that address on other side has correct balance

    logger.info('test');
    // cowAddress -> account 0 from regtest (account that has enough RBTC to perform any transaction)
    ...
    // TODO: if call passes but sendTransaction
    // mint token in mainchain
    ...
    //         const multiSigData = sideMultiSigContract.methods.confirmTransaction(nextTransactionCount).encodeABI();
    ...
    //     for (let i = 1; i < federatorKeys.length; i++) {
    //         const multiSigData = multiSigContract.methods.confirmTransaction(nextTransactionCount).encodeABI();
    //         await transactionSender.sendTransaction(config.mainchain.multiSig, multiSigData, 0, federatorKeys[i], true);
    //     }
    // }
    // TODO: rename to NFT bla
    // let destinationAnotherTokenAddress = await getDestinationTokenAddress(destinationBridgeContract,
    //   anotherTokenAddress, sideMultiSigContract, destinationTransactionSender, config, destinationLoggerName);
    //
