...
const abiValidators = require('../../../abis/Validators.json');
...
module.exports = class Validator {
    constructor(config, logger, Web3 = web3) {
        ...
        this.validatorsContract = new this.sideWeb3.eth.Contract(abiValidators, this.config.sidechain.validators);
    ...
    async run() {
        ...
        while(retries > 0) {
            try {
                ...
                console.log(err)
                this.logger.error(new Error('Exception Running Validator'), err);
                retries--;
    ...
    async _processLogs(logs, toBlock) {
        try {
            ...
            for(let log of logs) {
                ...
                let transactionId = await this.validatorsContract.methods.getTransactionId(
                    tokenAddress,
                    { sender,
                    receiver,
                    amount,
                ...
                let wasProcessed = await this.validatorsContract.methods.transactionWasProcessed(transactionId).call();
                if (!wasProcessed) {
                    let hasVoted = await this.validatorsContract.methods.hasVoted(transactionId).call({from: from});
                    if(!hasVoted) {
    ...
    async _voteTransaction(tokenAddress, sender, receiver, amount, symbol, blockHash, transactionHash, logIndex, decimals, granularity) {
        try {
            ...
            let txId = await this.validatorsContract.methods.getTransactionId(
                tokenAddress,
                {sender,
                receiver,
                amount,
            ...
            let member = await this.validatorsContract.methods.isMember(from).call();
            this.logger.info(`isMember ${member}`);
            this.logger.info(`this.validatorsContract.address ${this.validatorsContract._address}`);
            let validatorsAddress = await this.sideBridgeContract.methods.getValidators().call()
            this.logger.info(`getValidators ${validatorsAddress}`);
            let result = await this.validatorsContract.methods.voteTransaction(
                tokenAddress,
                {sender,
                receiver,
                amount,
            ...
            let txData = await this.validatorsContract.methods.voteTransaction(
                tokenAddress,
                {sender,
                receiver,
                amount,
            ...
            await transactionSender.sendTransaction(this.validatorsContract.options.address, txData, 0, this.config.privateKey);
            this.logger.info(`Voted transaction:${transactionHash} of block: ${blockHash} token ${symbol} to Validators Contract with TransactionId:${txId}`);
            return true;
        } catch (err) {
