 ...
 * Example invocation:
 *
 * node fundValidators.js "http://localhost:8545" "key1, key2" "pkey" "100000"
 *
 */
...
if (scriptPath.indexOf('fundValidators') !== -1) {
    keys = keys.replace(/ /g, '').split(',') || [];
    fundValidators(host, keys, privateKey, amount);
}

async function fundValidators(host, keys, privateKey, amount) {
